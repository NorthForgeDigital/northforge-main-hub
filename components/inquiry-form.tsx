...
export function InquiryForm() {
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); setPreviewComplete(true) }} className="grid gap-5 rounded-2xl border border-border bg-card p-5 sm:p-8">
      ...
      {previewComplete && <div role="status" className="rounded-xl border border-primary/30 bg-primary/5 p-4 text-sm"><strong className="text-foreground">Your request appears ready for review.</strong><p className="mt-2 text-muted-foreground">This preview does not transmit or store your information. Secure CRM submission will activate only after the NorthForge privacy contact, data destination and automated response workflow are verified.</p></div>}
